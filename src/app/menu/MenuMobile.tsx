...
export default function MenuMobile() {
  ...
  return (
    <section className="bg-mobilebg flex w-full max-w-[1300px] flex-row">
      <div className="flex w-full flex-col gap-0 md:w-4/6">
        <div className="bg-mobilebg flex h-fit w-full flex-col gap-2 pb-2 pt-5 md:hidden">
          <div className="flex w-full items-center justify-between px-5 py-2">
            <div
              onClick={() => router.back()}
              className="text-menusecondary p-0"
            >
        ...
        {/* Categories */}
        <div className="bg-mobilebg sticky top-0 z-40 flex items-center px-4 py-2 md:z-10">
          <div
            ref={categoryNavRef}
                ...
                <Button
                  ...
                  className={cn(
                    ...
                    activeCategory === category._id
                      ? "bg-menuprimary hover:bg-buttonhover hover:text-menuforeground text-[#fff]"
                      : "border-menuprimary bg-itembackground text-menuprimary hover:text-menuprimary border-[1px] hover:bg-transparent",
          ...
          <div className="flex flex-col gap-2 md:hidden">
            {sorted.map((category) => (
              ...
              >
                <h2
                  className={cn(
                    "text-menuprimary pb-3 text-2xl font-bold",
                    category.items.length === 0 && "hidden pb-0",
                  )}
                >
                  {category.name}
      ...
      {restaurant?.onlineOrder &&
        ...
        cartItems.length > 0 && (
          <Link
            className={cn(
              "bg-menuprimary fixed bottom-0 left-0 z-30 flex h-14 w-full items-center justify-between px-3 md:hidden",
            )}
            href="/cart"
          >
            <p className="text-menuforeground w-full text-center text-lg font-semibold uppercase">
